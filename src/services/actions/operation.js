...
export const getOperationsByTypeDynamically = createAsyncThunk(
  ...
  async function (type, { getState, rejectWithValue }) {
    try {
      ...
      const isFailed =
        type === "expenses" ? store.operationReducer.statistStatus.isFailedExpenses : store.operationReducer.statistStatus.isFailedIncome;
      const dates =
  ...
  async function (info, { getState, rejectWithValue, dispatch }) {
    try {
      ...
      if (info.isStatistic) {
        const store = getState();
        const dates =
          info.type === "expenses" ? store.operationReducer.statisticExpensesInfo.dates : store.operationReducer.statisticIncomeInfo.dates;
        const form = {
          fromDate: dates.from,
          toDate: dates.to,
          type: info.type,
  ...
  async function (form, { getState, rejectWithValue, dispatch }) {
    try {
      ...
      if (form.isStatistic) {
        const store = getState();
        const dates =
          form.type === "expenses" ? store.operationReducer.statisticExpensesInfo.dates : store.operationReducer.statisticIncomeInfo.dates;
        const data = {
          fromDate: dates.from,
          toDate: dates.to,
          type: form.type,
