      ...
      {data.length !== 0 && !isLoading && (
        <>
          <div className={styles.doughnut__container}>
            ...
            <ToolTip className={styles.center_tooltip} tooltip={`₽ ${Number(sum).toFixed(2)}`.length >= 12 ? `₽ ${sum}` : null}>
              <div className={styles.doughnut__sum}>{`₽ ${Number(Number(sum).toFixed(2))}`}</div>
            </ToolTip>
          </div>
